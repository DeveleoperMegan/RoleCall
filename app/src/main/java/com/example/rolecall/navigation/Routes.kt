package com.example.rolecall.navigation

object Routes {
    const val LOGIN = "login"
    const val SIGNUP = "signup"
    const val FORGOT_PASSWORD = "forgot_password"
    const val CHANGE_PASSWORD = "change_password"
    const val EDIT_PROFILE = "edit_profile"
    const val NOTIFICATION_SETTINGS = "notification_settings"
    const val PROFILE = "profile"
    const val UPLOAD = "upload"
    const val RESULTS = "results"
    const val JOB_DETAIL = "job_detail/{jobJson}"
    const val HISTORY = "history"

    const val ONBOARDING = "onboarding"
    const val MATCHING_ANIMATION = "matching_animation/{resumeId}"
    const val PREVIEW = "preview/{resumeId}"
    const val PREVIEW_LOCAL = "preview_local/{resumeId}"
    const val HOME = "home"
    const val SEARCH_RESULTS = "search_results"
    const val WEB_SEARCH_ANIMATION = "web_search_animation/{query}"
    const val APPLICATIONS = "applications"
    const val RESUME_BUILDER = "resume_builder"
    const val GENERATED_RESUME_PREVIEW = "generated_resume/{resumeId}"
    const val MY_RESUMES = "my_resumes"
}